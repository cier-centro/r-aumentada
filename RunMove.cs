...
public class RunMove : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        if (perro.ready && !Personero.pillado)
        {
            ...
            if (Boton.up && valid(Vector2.up))
            {
                dest = (Vector2)transform.position + Vector2.up;
                RunMove.dir = "arriba";
            }
            if (Boton.right && valid(Vector2.right))
            {
                dest = (Vector2)transform.position + Vector2.right;
                RunMove.dir = "derecha";
            }
            if (Boton.down && valid(-Vector2.up))
            {
                dest = (Vector2)transform.position - Vector2.up;
                RunMove.dir = "abajo";
            }
            if (Boton.left && valid(-Vector2.right))
            {
                dest = (Vector2)transform.position - Vector2.right;
                RunMove.dir = "izquierda";
            }
